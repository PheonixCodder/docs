...
<AccordionGroup>
  ...
  <Accordion title="Data synchronization">
    ...
    - **Products & Inventory** — SKUs, prices, stock levels, descriptions
      - Atomic sync: Products and inventory updated together
      - Delta detection: Only updates changed inventory
      - Batch operations: Efficient bulk inserts
    
    - **Orders & Order Items** — Order details, line items, fulfillment
      - Includes customer info, payment status
      - Links to products via SKU
      - Tracks fulfillment status
    
    - **Returns** — Return requests, refunds (selective platforms)
      - Supported: Amazon, Walmart, Shopify, Target
      - Not supported: TikTok, Faire, Warehance

    **Sync modes:**
    - **Full sync** — Fetches all data (first sync or no timestamp)
    - **Delta sync** — Fetches only changed data using `since` parameter
    - **Webhook sync** — Real-time updates (Shopify, TikTok, Walmart)

    **Sync triggers:**
    - Manual: Via API endpoint or dashboard
    - Scheduled: Via `TasksService.pollAllActiveStores()`
    - Webhook: Automatic on platform events
  </Accordion>
  ...
  <Accordion title="Rate limiting">
    Respects platform rate limits with adaptive strategies:

    **Platform limits:**
    - **Shopify:** 2 requests/second (REST), cost-based (GraphQL)
    - **TikTok:** 10 requests/second (varies by endpoint)
    - **Faire:** 100 requests/minute
    - **Walmart:** 5000 requests/day
    - **Amazon:** Varies by endpoint (SP-API throttling)

    **Mitigation strategies:**
    - Exponential backoff with jitter (prevents thundering herd)
    - Automatic retry on 429 errors (max 5 attempts)
    - Request queuing via BullMQ
    - Adaptive batch sizing (Walmart: 3 items/batch with 500ms delay)
    - Delta sync to reduce API calls
    - Webhook usage where available
  </Accordion>
</AccordionGroup>
