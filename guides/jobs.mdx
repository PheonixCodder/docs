...
Each platform has unique sync logic in the ProductsProcessor:

### Shopify sync flow

```typescript
private async processShopifyProducts(service: ShopifyService, store) {
  ...
  const productIdRows = await this.productsRepo
    .getProductIdsBySkusInBatches(store.id, skus, 'shopify');

  // 4. Fetch inventory with delta sync
  const inventoryItems = await service.fetchInventory(since);

  // 5. Delta comparison and upsert
  const existingInventory = await this.inventoryRepo.getBySkus(skus, store.id);
...
```typescript
private async processWalmartProducts(service: WalmartService, store) {
  // 1. Fetch all products
  const walmartProducts = await service.getProducts();

  // 2. Upsert products
  const productInserts = mapWalmartProductToDB(walmartProducts, store.id);
  await this.productsRepo.insertProducts(productInserts);

  // 3. Adaptive inventory fetch (rate-limited)
  await fetchInventoryAdaptive(productInserts, {
    batchSize: 3,
    initialDelayMs: 500,
    maxRetries: 3,
    handler: async (product) => {
      const inventory = await service.getInventory(product);
      // Delta check and upsert
    },
  });
}
```

**Adaptive fetching** prevents rate limiting by:
- Processing in small batches (3 items)
- Adding delays between requests (500ms)
...
private async processTiktokProducts(service: TikTokService, store) {
  ...
  // 3. Fetch inventory in batches (max 50 product IDs per request)
  ...
  for (let i = 0; i < productIds.length; i += 50) {
    ...
    // Process inventory with delta detection
    for (const inv of inventories) {
      // Map and compare with existing
    }
  }
}
```
...
- Maximum 50 product IDs per inventory request
- Requires batching for large catalogs

### Amazon sync flow

```typescript
private async processAmazonProducts(service: AmazonService, store) {
  // 1. Fetch listings report
  ...
  // 3. Fetch FBA inventory summaries
  const inventorySummaries = await service.getInventorySummaries(store, since);

  // 4. Delta comparison
  for (const summary of inventorySummaries) {
    const existing = existingInventory[summary.sellerSku];
...
- Uses Merchant Listings Report for products
- FBA Inventory API for stock levels
