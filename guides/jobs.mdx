...
<CardGroup cols={3}>
  <Card title="Products queue" icon="box">
    Synchronizes product data from marketplaces
  </Card>
  <Card title="Orders queue" icon="receipt">
    Synchronizes order data from marketplaces
  </Card>
  <Card title="Returns queue" icon="rotate-left">
    Synchronizes return data from marketplaces
  </Card>
</CardGroup>
...
Jobs are configured with retry logic and cleanup policies:
...
```typescript
{
  ...
  removeOnFail: {
    age: 14 * 24 * 60 * 60,      // Remove after 14 days
  },
}
```
