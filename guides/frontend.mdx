...
CrossConnect-Phos uses **shadcn/ui** components with custom extensions:

**Base components** (`components/ui/`):
- All shadcn/ui primitives (button, card, dialog, table, etc.)
- Custom components: `spinner.tsx`, `empty.tsx`, `field.tsx`

**Custom components:**

```tsx components/data-display/VirtualizedDataTable.tsx
// High-performance table for large datasets
import { VirtualizedDataTable } from '@/components/data-display/VirtualizedDataTable';
...
```tsx components/data-display/StatusBadge.tsx
// Platform-aware status badges
import { StatusBadge } from '@/components/data-display/StatusBadge';

<StatusBadge 
  status={store.auth_status}  // 'active' | 'inactive'
  platform={store.platform}
/>
```

```tsx components/layout/PageContainer.tsx
...
CrossConnect-Phos uses Zustand for global state management:
...
interface DashboardStore {
  activeStore: Database['public']['Tables']['stores']['Row'] | null;
  setActiveStore: (store: Database['public']['Tables']['stores']['Row'] | null) => void;
}
...
export default async function DashboardPage() {
  ...
  return <DashboardView stores={stores} />;
}
```
